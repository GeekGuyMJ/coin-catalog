...
const CACHE = 'coin-catalog-public-v74';
const ASSETS = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('fetch', (event) => {
  // Never cache API or WebDAV calls — always go to network (avoids serving stale
  // backend responses like a broken /api/spot_history from a previous deploy).
  const _u = new URL(event.request.url);
  if (event.request.method !== 'GET' || _u.pathname.startsWith('/api/') || _u.pathname.startsWith('/dav/')) {
    return; // let the browser handle it normally (no cache.put)
  }
